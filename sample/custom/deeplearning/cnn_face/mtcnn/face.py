...
class MTCNN(object):
    def __init__(self, sess, model_dir):
        self.pnet, self.rnet, self.onet = self.load_caffe_model(sess, model_dir)

    def load_caffe_model(self, sess, model_dir):
        if not model_dir:
    ...
    def detect_face(self, img, minsize, threshold, factor):
    ...
    def bulk_detect_face(self, images, detection_window_size_ratio, threshold, factor):
        ...
        for index, scales in enumerate(all_scales):
            ...
            for scale in scales:
                ...
                im_data = self.imresample(images[index], (hs, ws))
                im_data = (im_data - 127.5) * 0.0078125
        ...
        for resolution in images_obj_per_resolution:
            ...
            outs = self.pnet(images_per_resolution)
            ...
            for index in range(len(outs[0])):
                ...
                boxes, _ = self.generateBoundingBox(out1[:, :, 1].copy(), out0[:, :, :].copy(), scale, threshold[0])
        ...
        for index, image_obj in enumerate(images_with_boxes):
            ...
            if numbox > 0:
                h = images[index].shape[0]
                w = images[index].shape[1]
                pick = self.nms(image_obj['total_boxes'].copy(), 0.7, 'Union')
                ...
                image_obj['total_boxes'] = self.rerec(image_obj['total_boxes'].copy())
                image_obj['total_boxes'][:, 0:4] = np.fix(image_obj['total_boxes'][:, 0:4]).astype(np.int32)
                dy, edy, dx, edx, y, ey, x, ex, tmpw, tmph = self.pad(image_obj['total_boxes'].copy(), w, h)
        ...
        for index, image_obj in enumerate(images_with_boxes):
            ...
            if image_obj['total_boxes'].shape[0] > 0:
                h = images[index].shape[0]
                w = images[index].shape[1]
                pick = self.nms(image_obj['total_boxes'], 0.7, 'Union')
                image_obj['total_boxes'] = image_obj['total_boxes'][pick, :]
                image_obj['total_boxes'] = self.bbreg(image_obj['total_boxes'].copy(), np.transpose(mv[:, pick]))
                image_obj['total_boxes'] = self.rerec(image_obj['total_boxes'].copy())
        ...
        for index, image_obj in enumerate(images_with_boxes):
            ...
            if image_obj['total_boxes'].shape[0] > 0:
                image_obj['total_boxes'] = self.bbreg(image_obj['total_boxes'].copy(), np.transpose(mv))
                pick = self.nms(image_obj['total_boxes'].copy(), 0.7, 'Min')
                image_obj['total_boxes'] = image_obj['total_boxes'][pick, :]
