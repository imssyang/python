...
def yolo_head(feats, anchors, num_classes):
    ...
    print(type(conv_index), type(feats))
...
def yolo_eval(yolo_outputs,
    ...
    print('boxes * image_dims', type(boxes), type(image_dims))
    boxes = boxes * image_dims

    # TODO: Something must be done about this ugly hack!
    max_boxes_tensor = K.variable(max_boxes, dtype='int32')
    bbb = tf.compat.v1.variables_initializer([max_boxes_tensor])
    print('--------max_boxes_tensor', max_boxes_tensor)
    print('--------bbb', bbb)
    K.get_session().run(bbb)
