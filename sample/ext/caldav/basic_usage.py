...
def add_stuff_to_calendar_demo(calendar):
    ...
    try:
        ...
        print(f"Supported component types: {acceptable_component_types}")
...
def search_calendar_demo(calendar):
    ...
    print(f"Getting all children from the calendar {all_objects=}")
    children = calendar.children()
    print(f"Getting all events from the calendar {children=}")
    events = calendar.events()
    print(f"Getting all todos from the calendar {events=}")
    #tasks = calendar.todos()
    #assert len(events) + len(tasks) == len(all_objects)
    print(
        f"Found {len(events)} events and tasks which is {len(all_objects)}"
    )
