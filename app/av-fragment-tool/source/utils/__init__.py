...
class VXCodeUtility:
    ...
    def setup_method(self, method):
        signal.signal(signal.SIGCHLD, VXCodeUtility._signal_handler)
    ...
    def delete_file(path):
        result = VXCodeUtility._run_subprocess(f"rm -rf {path}", shell=True)
        if result["retcode"]:
    ...
    @staticmethod
    def exec_subprocess(cmd, timeout=None, shell=False):
        result = VXCodeUtility._run_subprocess(cmd, timeout, shell)
        if result["retcode"]:
    ...
    def _signal_handler(signum, frame):
        try:
            ...
            logging.info(f"childpid: {cpid} exit status: {status} when test")
        except:
            pass
